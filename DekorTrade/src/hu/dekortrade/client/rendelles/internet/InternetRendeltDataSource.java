...
public class InternetRendeltDataSource extends GwtRpcDataSource {
 ...
 new AsyncCallback<List<RendeltSer>>() {
	...
	private static void copyValues(RendeltSer from, ListGridRecord to) {
		to.setAttribute(InternetConstants.INTERNETRENDELT_ROVIDNEV,
				from.getRovidnev());
		to.setAttribute(InternetConstants.INTERNETRENDELT_RENDELES,
				from.getRendeles());
		to.setAttribute(InternetConstants.INTERNETRENDELT_DATUM,
				from.getDatum());
	}

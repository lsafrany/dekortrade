...
public class Cikktorzs {
	...
	public Canvas process(final IButton extIButton,
		...
		szuresIButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				Criteria criteria = new Criteria();
				page = 0;
				criteria.setAttribute(CikktorzsConstants.CTORZS_PAGE, page);
				criteria.setAttribute(CikktorzsConstants.CIKK_FOTIPUS,
						fotipusSelectItem.getValueAsString());
				criteria.setAttribute(CikktorzsConstants.CIKK_ALTIPUS,
						altipusSelectItem.getValueAsString());
				criteria.setAttribute(CikktorzsConstants.CIKK_CIKKSZAM,
		...
		ctorzsGrid.addRecordClickHandler(new RecordClickHandler() {
			public void onRecordClick(RecordClickEvent event) {
				...
				if (ctorzsGrid.getSelectedRecord().getAttributeAsInt(
						CikktorzsConstants.CIKK_KEPEK) > 0)
				...
				final KepDataSource kepDataSource = new KepDataSource() {
					...
					protected Object transformRequest(DSRequest dsRequest) {
						...
						dsRequest.setAttribute(
								...
								ctorzsGrid.getSelectedRecord().getAttribute(
										CikktorzsConstants.CIKK_SZINKOD));
						dsRequest.setAttribute(CikktorzsConstants.KEP_SORSZAM,
								sorszam);
						return super.transformRequest(dsRequest);
					}
							...
							public void execute(Boolean value) {
								if (value != null && value) {
									sorszam = tileGrid
											.getSelectedRecord()
											.getAttribute(
													CikktorzsConstants.KEP_SORSZAM);
	...
	void ctorzsEdit(final CikktorzsDataSource dataSource,
			final ListGrid listGrid, boolean uj,
			final CikkSelectsSer cikkSelectsSer) {
		...
		editForm.getField(CikktorzsConstants.CIKK_AKCIOS).setColSpan(3);

		editForm.getField(CikktorzsConstants.CIKK_LEIRAS).setColSpan(3);
		editForm.getField(CikktorzsConstants.CIKK_MEGJEGYZES).setColSpan(3);

		editForm.getField(CikktorzsConstants.CIKK_KEPEK).setVisible(false);

		if (uj) {
			editForm.editNewRecord();
			editForm.getField(CikktorzsConstants.CIKK_FOTIPUS).setDefaultValue(
					"1");
			editForm.getField(CikktorzsConstants.CIKK_ALTIPUS).setDefaultValue(
					"1");
			editForm.getField(CikktorzsConstants.CIKK_GYARTO).setDefaultValue(
					"1");
			editForm.getField(CikktorzsConstants.CIKK_MERTEKEGYSEG)
					.setDefaultValue("DB");
		} else {
			editForm.getField(CikktorzsConstants.CIKK_CIKKSZAM).setCanEdit(
					false);
			editForm.getField(CikktorzsConstants.CIKK_SZINKOD)
					.setCanEdit(false);
			editForm.editSelectedData(listGrid);
		}

		LinkedHashMap<String, String> altipus = cikkSelectsSer.getTipus().get(
				editForm.getField(CikktorzsConstants.CIKK_FOTIPUS).getValue());
		if (altipus == null)
			editForm.getField(CikktorzsConstants.CIKK_ALTIPUS).setValueMap(
					new LinkedHashMap<String, String>());
		else
			editForm.getField(CikktorzsConstants.CIKK_ALTIPUS).setValueMap(
					altipus);

		editForm.getField(CikktorzsConstants.CIKK_FOTIPUS).addChangedHandler(
				new ChangedHandler() {
					public void onChanged(ChangedEvent event) {
						editForm.getField(CikktorzsConstants.CIKK_ALTIPUS)
								.clearValue();
						LinkedHashMap<String, String> altipus = cikkSelectsSer
								.getTipus().get(event.getValue());
						if (altipus == null)
	...
	void upload(final ListGrid ctorzsGrid) {
		...
		uploadForm.setAction(GWT.getModuleBaseURL()
				...
				+ "&szinkod="
				+ ctorzsGrid.getSelectedRecord().getAttribute(
						CikktorzsConstants.CIKK_SZINKOD) + "&random=" + random);
																		...
																		ctorzsGrid
																				...
																				.setAttribute(
																						...
																						ctorzsGrid
																								.getSelectedRecord()
																								.getAttributeAsInt(
																										CikktorzsConstants.CIKK_KEPEK) + 1);
																		ctorzsGrid

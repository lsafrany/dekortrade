...
public class JogDataSource extends GwtRpcDataSource {
	...
	public JogDataSource() {
		...
		field = new DataSourceEnumField(RendszerConstants.JOG_NEV,
				systemLabels.jog_nev());
		field.setPrimaryKey(true);
		field.setCanEdit(false);
		field.setValueMap(ClientConstants.getMenu());
		addField(field);

		field = new DataSourceBooleanField(RendszerConstants.JOG_JOG,
	...
	protected void executeFetch(final String requestId,
		...
		dekorTradeService.getJog(
				request.getCriteria().getAttributeAsString(
						RendszerConstants.FELHASZNALO_ROVIDNEV),
	...
	private static void copyValues(JogSer from, ListGridRecord to) {
		to.setAttribute(RendszerConstants.JOG_NEV, from.getNev());
		to.setAttribute(RendszerConstants.JOG_JOG, from.getJog());
	}

	private static void copyValues(ListGridRecord from, JogSer to) {
		to.setNev(from.getAttributeAsString(RendszerConstants.JOG_NEV));
		to.setJog(from.getAttributeAsBoolean(RendszerConstants.JOG_JOG));
	}

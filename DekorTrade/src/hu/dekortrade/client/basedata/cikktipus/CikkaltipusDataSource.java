package hu.dekortrade.client.basedata.cikktipus;
...
public class CikkaltipusDataSource extends GwtRpcDataSource {
	...
	protected void executeFetch(final String requestId,
		...
		dekorTradeService.getCikkaltipus(request.getCriteria().getAttributeAsString(
				CikktipusConstants.CIKKALTIPUS_FOKOD),new AsyncCallback<List<CikkaltipusSer>>() {
	...
	protected void executeAdd(final String requestId, final DSRequest request,
		...
		dekorTradeService.addCikkaltipus(cikkaltipusSer, new AsyncCallback<CikkaltipusSer>() {
			public void onFailure(Throwable caught) {
				if (caught instanceof SQLExceptionSer)
					response.setAttribute(ClientConstants.SERVER_SQLERROR,
							caught.getMessage());
	...
	protected void executeUpdate(final String requestId,
		...
		CikkaltipusSer cikkaltipusSer = new CikkaltipusSer();
		copyValues(rec, cikkaltipusSer);
		dekorTradeService.updateCikkaltipus(cikkaltipusSer, new AsyncCallback<CikkaltipusSer>() {
			public void onFailure(Throwable caught) {
				if (caught instanceof SQLExceptionSer)
					response.setAttribute(ClientConstants.SERVER_SQLERROR,
							caught.getMessage());
	...
	private static void copyValues(ListGridRecord from, CikkaltipusSer to) {
		to.setFokod(from.getAttributeAsString(CikktipusConstants.CIKKALTIPUS_FOKOD));
